...
export const activities = pgTable(
  ...
  (table) => {
    return {
      ...
      isActiveIdx: index("activities_is_active_idx").on(table.isActive),
...
export const enquiries = pgTable(
  ...
  {
    id: uuid("id").defaultRandom().primaryKey(),
    siteId: varchar("site_id", { length: 64 }).notNull(), // Tracks which brand/domain
    sourcePage: varchar("source_page", { length: 255 }), // Page where enquiry originated
    destinationId: varchar("destination_id", { length: 64 }), // Optional: if enquiry is for specific destination/activity
    activityId: uuid("activity_id").references(() => activities.id, { onDelete: "set null" }), // Optional: specific activity
    name: varchar("name", { length: 180 }).notNull(),
    email: varchar("email", { length: 255 }).notNull(),
    phoneCountryCode: varchar("phone_country_code", { length: 8 }),
    phoneNumber: varchar("phone_number", { length: 32 }),
    message: text("message"),
    status: varchar("status", { length: 32 }).default("new").notNull(), // new, contacted, qualified, closed
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
      .notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true })
  ...
  (table) => ({
    siteIdx: index("enquiries_site_idx").on(table.siteId),
    destinationIdx: index("enquiries_destination_idx").on(table.destinationId),
